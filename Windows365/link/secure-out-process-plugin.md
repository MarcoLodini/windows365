...
1. The RDP client process loads the RDP plugins out of proc, that is, as an independent process separate from the RDP client and
...
The following diagram shows a high-level explaination of how the RDP clients interact with the RDP plugins in an out of proc model.
...
A mechanism used for transferring data/COM objects during an out of proc communication between COM client and the COM server across address spaces, such as between different processes.