...
- To help protect the kernel from malicious code injection, Windows 465 Link includes:
  ...
  - Silicon-assisted security features like [Virtualization-based Security](/windows-hardware/design/device-experiences/oem-vbs) (VBS) and [hypervisor-protected code integrity](/windows/security/hardware-security/enable-virtualization-based-protection-of-code-integrity?tabs=security) (HVCI).
...
Windows 365 Link has a small, purpose-built Windows-based operating system called Windows CPC with features that help reduce the attack surface. This operating system includes only the essential components needed to securely authenticate users against Entra ID and connect them to their Windows 365 Cloud PC.
OS based security features are enabled by default and can’t be turned off, like:

- [BitLocker](/windows/security/operating-system-security/data-protection/bitlocker/).
...
- Security baseline policies, which remove the need to define and apply such configurations to the endpoint.

Other features that help to reduce the attack surface include:
...
- Windows 365 Link provides a password-less experience with a FIDO2 security key and web sign-in as the only credential providers available on the device for sign-in.
