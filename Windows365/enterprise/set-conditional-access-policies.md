...
- Microsoft Intune. The following steps explain this process. For more information, see [Learn about Conditional Access and Intune](/mem/intune/protect/conditional-access).
...
11. If you want to test your policy first, under **Enable policy**, select **Report-only**. If you set it to **On**, the policy is applied as soon as you create it.
...
> - Users must reauthenticate if there's a network disruption that forces the session to be re-established after the sign-in frequency you've configured. This reauthentication can lead to more frequent authentication requests on unstable networks.
